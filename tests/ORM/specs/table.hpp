...
void init(Moka::Context &it) {
  it.describe("Static table functionalities", [](Moka::Context &it) {
    it.describe_many<User, UserWithNonProperties, UserAsClass,
                     UserAsClassWithNonProperties,
                     UserWithStaticConstexprMembers>(
        "for %s",
        {"standard struct", "struct with custom non-DB data",
         "class with public properties", "class with custom non-DB data",
         "struct with static constexpr members"},
        []<typename T>(Moka::Context &it) {
          it.should("correctly find the 1st property", []() {
            must_be_equal(Table<T>::get_index_by_column("id"), 0ull,
                          "get_index_by_column could not identify correctly "
                          "the index of the first column.");
          });
          it.should("correctly find the 2nd property", []() {
            must_be_equal(Table<T>::get_index_by_column("name"), 1ull,
                          "get_index_by_column could not identify correctly "
                          "the index of the second column.");
          });
        });
    it.describe_many<UserWithNonPropertiesFirst,
                     UserAsClassWithNonPropertiesFirst>(
        "for %s",
        {"struct with custom non-DB data first",
         "struct with custom non-DB data first"},
        []<typename T>(Moka::Context &it) {
          it.should("correctly find the 1st property", []() {
            must_be_equal(Table<T>::get_index_by_column("id"), 1ull,
                          "get_index_by_column could not identify correctly "
                          "the index of the first column.");
          });
          it.should("correctly find the 2nd property", []() {
            must_be_equal(Table<T>::get_index_by_column("name"), 2ull,
                          "get_index_by_column could not identify correctly "
                          "the index of the second column.");
          });
        });
