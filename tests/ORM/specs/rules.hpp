	...
	void init(Moka::Context& it)
	{
		it.describe("Compile-time rule optimization", [](Moka::Context& it) {
			it.describe("User::id > 5", [](Moka::Context& it) {
				...
				it.should("should have operation \'>\'", [rule]() { must_be_equal(rule.operation, ">", "Operation doesn't match."); });
			...
			it.describe(
				...
				[](Moka::Context& it) {
					...
					it.should(
						...
						"\"user\"))",
						[rule]() {
							constexpr auto expected_rule = ((P<&Post::author_id> < 1) || (P<&User::id> > 5) || (P<&Post::id> >= 5)) &&
								((P<&Post::content> != "text") == (P<&User::name> == "user"));
