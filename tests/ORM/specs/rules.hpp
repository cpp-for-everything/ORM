...
namespace RulesTests
{
	...
	constexpr auto replace_with_value(auto mem_ptr)
	{
		...
		else if constexpr (std::is_same_v<decltype(mem_ptr), std::string>)
		{
			return mem_ptr;
		}
		else if constexpr (std::is_same_v<decltype(mem_ptr), std::string_view>)
		{
			return mem_ptr;
		}
		else if constexpr (std::is_same_v<decltype(mem_ptr), const char*>)
		{
			return std::string(mem_ptr);
		}

		else if constexpr (mem_ptr.template equals<&User::id>())
		{
			return 1;
		}
		else if constexpr (mem_ptr.template equals<&User::name>())
		{
			return "Username";
		}
		else if constexpr (mem_ptr.template equals<&Post::id>())
		{
			return 2;
		}
		else if constexpr (mem_ptr.template equals<&Post::author_id>())
		{
			return 3;
		}
		else if constexpr (mem_ptr.template equals<&Post::content>())
		{
			return "PostContent";
		}
	}
	...
	void init(Moka::Context& it)
	{
		it.describe("Compile-time rule optimization", [](Moka::Context& it) {
			it.describe("User::id > 5", [](Moka::Context& it) {
				...
				it.should("should have operation \'>\'", [rule]() { must_be_equal(rule.operation, details::Greater, "Operation doesn't match."); });
			...
			it.describe("!(User::id > 5)", [](Moka::Context& it) {
				...
				it.describe("should be simplified to User::id <= 5", [rule](Moka::Context& it) {
					...
					it.should("should have operation \'<=\'", [rule]() { must_be_equal(rule.operation, details::Less_or_equal, "Operation doesn't match."); });
				});
			...
			it.describe("!(User::id > 5) && (User::id > 5)", [](Moka::Context& it) {
				...
				it.describe("should be simplified to User::id <= 5 && (User::id > 5)", [rule](Moka::Context& it) {
					...
					it.should("should have operation \'&&\'", [rule]() { must_be_equal(rule.operation, details::And, "Operation doesn't match."); });
				});
				it.should("be evaluated as false when User::id is 1", [rule]() {
					constexpr bool eval = evaluate_rule(rule);
			...
			it.describe(
				...
				[](Moka::Context& it) {
					...
					it.should(
						"be evaluated as false when User::id=1; User::name=\"Username\"; Post::id=2; Post::author_id=3; Post::content=\"PostContent\"",
						[rule]() {
							constexpr bool eval = evaluate_rule(rule);
