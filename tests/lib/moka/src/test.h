...
namespace Moka
{
	template <typename... Ts> std::string format(std::string format_string, Ts... args)
	{
		std::string answer;
...
namespace Moka
{
	...
	class Context : public Base
	{
		...
		void describe_many_impl(std::string name, const std::vector<std::string>& placeholders, auto fn, std::index_sequence<ind...>)
		{ // fn = void<T>(Context&)
			((mMembers.push_back(
				 new Context("", Moka::format(name, placeholders[ind].c_str()), [fn](Context& it) -> void { fn.template operator()<Ts>(it); }))),
			 ...);
		}

		template <typename... Ts, size_t... ind>
		void describe_for_impl(std::string name, const std::tuple<std::pair<Ts, std::string>...>& params, auto fn, std::index_sequence<ind...>)
		{ // fn = void<T>(Context&)
			(mMembers.push_back(new Context(
				 "",
				 Moka::format(name, std::get<ind>(params).second.c_str()),
				 [fn, param = std::get<ind>(params).first](Context& it) -> void { fn.operator()(it, param); })),
			 ...);
		}

		public:
		template <typename... Ts> void describe_many(std::string name, const std::initializer_list<std::string>& placeholders, auto fn)
		{ // fn = void<T>(Context&)
			describe_many_impl<Ts...>(name, placeholders, fn, std::index_sequence_for<Ts...>{});
		}

		template <typename... Ts> void describe_for(std::string name, const std::tuple<std::pair<Ts, std::string>...>& params, auto fn)
		{ // fn = void<T>(Context&)
		...
		void should_many_impl(std::string name, const std::vector<std::string>& placeholders, Lambda fn, std::index_sequence<ind...>)
		{ // fn = void<T>(Context&)
			(mMembers.push_back(new Test("should " + Moka::format(name, placeholders[ind].c_str()), [&fn]() -> void { fn.template operator()<Ts>(); })), ...);
		}

		template <typename Lambda, typename... Ts, size_t... ind>
		void should_for_impl(std::string name, const std::tuple<std::pair<Ts, std::string>...>& params, Lambda fn, std::index_sequence<ind...>)
		{ // fn = void<T>(Context&)
			(mMembers.push_back(new Test(
				 "should " + Moka::format(name, std::get<ind>(params).second.c_str()),
				 [fn, param = std::get<ind>(params).first]() -> void { fn.operator()(param); })),
			 ...);
		}

		template <typename Lambda, auto... values, size_t... ind>
		void should_for_values_impl(std::string name, const std::vector<std::string> placeholders, Lambda fn, std::index_sequence<ind...>)
		{ // fn = void<var>()
			(mMembers.push_back(new Test("should " + Moka::format(name, placeholders[ind].c_str()), [fn]() -> void { fn.template operator()<values>(); })),
			 ...);
		}

		public:
		template <typename... Ts, typename Lambda> void should_many(std::string name, std::initializer_list<std::string> placeholders, Lambda fn)
		{
			should_many_impl<Lambda, Ts...>(name, placeholders, fn, std::index_sequence_for<Ts...>{});
		}

		template <typename... Ts, typename Lambda> void should_for(std::string name, std::tuple<std::pair<Ts, std::string>...> params, Lambda fn)
		{
			should_for_impl<Lambda, Ts...>(name, params, fn, std::index_sequence_for<Ts...>{});
		}

		template <auto... values, typename Lambda> void should_for_values(std::string name, std::initializer_list<std::string> placeholders, Lambda fn)
		{
