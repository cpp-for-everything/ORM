...
namespace fakeit
{
	...
	template <typename T> class is_ostreamable
	{
		...
		template <typename T1> static decltype(operator<<(std::declval<std::ostream&>(), std::declval<const T1>())) test(std::ostream& s, const T1& t);
#else
	...
	template <typename CharT, typename Traits> class is_ostreamable<std::basic_ios<CharT, Traits>& (*)(std::basic_ios<CharT, Traits>&)>
	{
		public:
		static const bool value = true;
	};

	template <typename CharT, typename Traits> class is_ostreamable<std::basic_ostream<CharT, Traits>& (*)(std::basic_ostream<CharT, Traits>&)>
	{
		public:
		static const bool value = true;
	};
...
namespace fakeit
{
	...
	struct NoMoreInvocationsVerificationEvent : public VerificationEvent
	{
		...
		NoMoreInvocationsVerificationEvent(std::vector<Invocation*>& allTheIvocations, std::vector<Invocation*>& anUnverifedIvocations)
			: VerificationEvent(VerificationType::NoMoreInvocations), _allIvocations(allTheIvocations), _unverifedIvocations(anUnverifedIvocations)
		{
		}
	...
	struct SequenceVerificationEvent : public VerificationEvent
	{
		...
		SequenceVerificationEvent(
			...
			int anExpectedCount,
			int anActualCount)
			: VerificationEvent(aVerificationType), _expectedPattern(anExpectedPattern), _actualSequence(anActualSequence), _expectedCount(anExpectedCount),
...
namespace fakeit
{
	...
	template <typename TARGET, typename SOURCE> FAKEIT_NO_DEVIRTUALIZE_ATTR TARGET union_cast(SOURCE source)
	{
...
namespace fakeit
{
	...
	#pragma GCC diagnostic pop
		...
		template <typename C> static typename std::enable_if<std::has_virtual_destructor<C>::value, unsigned int>::type getDestructorOffset()
		{
			VirtualOffsetSelector offsetSelctor;
		...
		template <typename C> static typename std::enable_if<std::has_virtual_destructor<C>::value, bool>::type hasVirtualDestructor()
		{
			return true;
		}
...
namespace fakeit
{
	...
	{
	#ifdef _WIN64
		RTTICompleteObjectLocator(const std::type_info& unused) : signature(0), offset(0), cdOffset(0), typeDescriptorOffset(0), classDescriptorOffset(0)
		{
			(void)unused;
		}
...
namespace fakeit
{
	...
	template <typename T1, typename T2, typename... types> class has_one_base<std::tr2::__reflection_typelist<T1, T2, types...>> : public std::false_type
	{
	};

	template <typename T1> class has_one_base<std::tr2::__reflection_typelist<T1>> : public has_one_base<typename std::tr2::direct_bases<T1>::type>
	{
	};
...
namespace fakeit
{
	namespace details
	{
		template <int instanceAreaSize, typename C, typename... BaseClasses> class FakeObjectImpl
		{
			public:
		...
		template <typename C, typename... BaseClasses> class FakeObjectImpl<0, C, BaseClasses...>
		{
			public:
			void initializeDataMembersArea()
			{
	...
	class FakeObject : public details::FakeObjectImpl<sizeof(C) - sizeof(VirtualTable<C, BaseClasses...>), C, BaseClasses...>
	{
...
namespace fakeit
{
	...
	struct InvocationHandlerCollection
	{
		...
		static InvocationHandlerCollection* getInvocationHandlerCollection(void* instance)
		{
			...
			InvocationHandlerCollection* invocationHandlerCollection = (InvocationHandlerCollection*)vt.getCookie(InvocationHandlerCollection::VT_COOKIE_INDEX);
...
namespace fakeit
{
	...
	template <typename C, typename... baseclasses> struct DynamicProxy
	{
		...
		DynamicProxy(C& inst)
			: instance(inst), originalVtHandle(VirtualTable<C, baseclasses...>::getVTable(instance).createHandle()), _methodMocks(VTUtils::getVTSize<C>()),
			  _offsets(VTUtils::getVTSize<C>()), _invocationHandlers(_methodMocks, _offsets)
		{
		...
		template <typename DATA_TYPE, typename... arglist> void stubDataMember(DATA_TYPE C::*member, const arglist&... initargs)
		{
			DATA_TYPE C::*theMember = (DATA_TYPE C::*)member;
			C& mock = get();
			DATA_TYPE* memberPtr = &(mock.*theMember);
			_members.push_back(
				std::shared_ptr<DataMemeberWrapper<DATA_TYPE, arglist...>>{new DataMemeberWrapper<DATA_TYPE, arglist...>(memberPtr, initargs...)});
		}
...
namespace fakeit
{
	...
	struct TupleDispatcher
	{
		...
		template <typename R, typename... ArgsF, typename... ArgsT, typename FunctionType> static R applyTuple(FunctionType&& f, std::tuple<ArgsT...>& t)
		{
			return apply_func<sizeof...(ArgsT)>::template applyTuple<R>(std::forward<FunctionType>(f), t);
		}

		template <typename R, typename... arglist, typename FunctionType> static R invoke(FunctionType&& func, const std::tuple<arglist...>& arguments)
		{
		...
		static void for_each(
			TupleType&&, FunctionType&, std::integral_constant<size_t, std::tuple_size<typename std::remove_reference<TupleType>::type>::value>)
		{
		}
		...
		static void for_each(
			TupleType1&&, TupleType2&&, FunctionType&, std::integral_constant<size_t, std::tuple_size<typename std::remove_reference<TupleType1>::type>::value>)
		{
		}
		...
		template <typename TupleType1, typename TupleType2, typename FunctionType> static void for_each(TupleType1&& t, TupleType2&& t2, FunctionType& f)
		{
...
namespace fakeit
{
	...
	{
		static const bool value = std::is_default_constructible<typename naked_type<C>::type>::value && !std::is_abstract<typename naked_type<C>::type>::value;
	};
...
namespace fakeit
{
	...
	template <typename ExpectedTRef> struct ComparisonMatcherCreatorBase
	{
		...
		template <typename T> ComparisonMatcherCreatorBase(T&& expectedRef) : _expectedRef(std::forward<T>(expectedRef))
		{
		}

		template <typename ActualT, typename = ExpectedT, typename = void> struct MatcherBase : public TypedMatcher<ActualT>
		{
		...
		struct MatcherBase<ActualT, U, typename std::enable_if<std::is_same<U, ExpectedT>::value && std::is_array<U>::value>::type>
	...
	namespace internal
	{
		...
		template <typename ExpectedTRef> struct EqMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;

			template <typename ActualT, typename = void> struct IsTypeCompatible : std::false_type
			{
			};
		...
		template <typename ExpectedTRef> struct GtMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct GeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct LtMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct LeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct NeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrEqMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrGtMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrGeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrLtMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrLeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef> struct StrNeMatcherCreator : public ComparisonMatcherCreatorBase<ExpectedTRef>
		{
			using ExpectedT = typename ComparisonMatcherCreatorBase<ExpectedTRef>::ExpectedT;
		...
		template <typename ExpectedTRef, typename ExpectedMarginTRef> struct ApproxEqCreator
		{
			using ExpectedT = typename naked_type<ExpectedTRef>::type;
			...
			struct IsTypeCompatible<
				ActualT,
				fk_void_t<decltype(std::abs(std::declval<ActualT>() - std::declval<ExpectedT>()) <= std::declval<ExpectedMarginT>())>> : std::true_type
			{
			};
...
namespace fakeit
{
	...
	template <typename... arglist> struct ArgumentsMatcherInvocationMatcher : public ActualInvocation<arglist...>::Matcher
	{
		...
		struct MatchingLambda
		{
			...
			{
				TypedMatcher<typename naked_type<A>::type>* matcher = dynamic_cast<TypedMatcher<typename naked_type<A>::type>*>(_matchers[index]);
				if (_matching)
				{
...
namespace fakeit
{
	...
	class RecordedMethodBody : public MethodInvocationHandler<R, arglist...>, public ActualInvocationsSource, public ActualInvocationsContainer
	{
		...
		struct MatchedInvocationHandler : ActualInvocationHandler<R, arglist...>
		{
			...
			MatchedInvocationHandler(typename ActualInvocation<arglist...>::Matcher* matcher, ActualInvocationHandler<R, arglist...>* invocationHandler)
		...
		MatchedInvocationHandler* buildMatchedInvocationHandler(
			typename ActualInvocation<arglist...>::Matcher* invocationMatcher, ActualInvocationHandler<R, arglist...>* invocationHandler)
		{
		...
		void addMethodInvocationHandler(typename ActualInvocation<arglist...>::Matcher* matcher, ActualInvocationHandler<R, arglist...>* invocationHandler)
		{
		...
		R handleMethodInvocation(const typename fakeit::production_arg<arglist>::type... args) override
		{
			...
			auto actualInvocation =
				new ActualInvocation<arglist...>(ordinal, method, std::forward<const typename fakeit::production_arg<arglist>::type>(args)...);
...
namespace fakeit
{
	...
	template <typename R, typename... arglist> struct MethodStubbingProgress
	{
		...
		template <typename U = R> typename std::enable_if<!std::is_reference<U>::value, MethodStubbingProgress<R, arglist...>&>::type Return(const R& r)
		{
		...
		template <typename... valuelist> MethodStubbingProgress<R, arglist...>& ReturnAndSet(R&& r, valuelist&&... arg_vals)
		{
			return Do(GetAssigner(std::forward<R>(r), std::forward<valuelist>(arg_vals)...));
		}

		template <typename... valuelist> void AlwaysReturnAndSet(R&& r, valuelist&&... arg_vals)
		{
		...
		GetAssigner(R&& r, valuelist&&... arg_vals)
		{
			...
			{
				public:
				Lambda(R&& r, valuelist&&... arg_vals) : vals_tuple{std::forward<R>(r), std::forward<valuelist>(arg_vals)...}
				{
				}
		...
		GetAssigner(R&& r, helper::ArgValue<T, N>... arg_vals)
		{
			...
			{
				public:
				Lambda(R&& r, helper::ArgValue<T, N>... arg_vals) : ret{std::forward<R>(r)}, vals_tuple{std::forward<helper::ArgValue<T, N>>(arg_vals)...}
				{
				}
	...
	template <typename... arglist> struct MethodStubbingProgress<void, arglist...>
	{
		...
		virtual MethodStubbingProgress<void, arglist...>& Do(std::function<void(const typename fakeit::test_arg<arglist>::type...)> method)
		{
		...
		template <typename... valuelist> MethodStubbingProgress<void, arglist...>& ReturnAndSet(valuelist&&... arg_vals)
		{
			return Do(GetAssigner(std::forward<valuelist>(arg_vals)...));
		}

		template <typename... valuelist> void AlwaysReturnAndSet(valuelist&&... arg_vals)
		{
	...
	namespace helper
	{
		...
		template <int max_index, int tuple_index> struct ArgValidator
		{
			template <typename... T, int... N> static void CheckPositions(const std::tuple<ArgValue<T, N>...> arg_vals)
			{
#if __cplusplus >= 201402L
		...
		typename std::enable_if<std::is_pointer<current_arg>::value, typename std::remove_pointer<current_arg>::type&>::type GetArg(current_arg&& t)
		{
			return *t;
		}

		template <typename current_arg> typename std::enable_if<!std::is_pointer<current_arg>::value, current_arg>::type GetArg(current_arg&& t)
		{
		...
		{
			template <typename... valuelist, typename... arglist> static void Assign(ArgumentsTuple<valuelist...>, arglist...)
			{
			}
		};
		...
		{
			template <typename current_arg, typename... T, int... N> static void AssignArg(current_arg&& p, std::tuple<ArgValue<T, N>...> arg_vals)
			{
			...
			private:
			template <typename T, typename U>
			static typename std::enable_if<std::is_convertible<U, decltype(GetArg(std::declval<T>()))>::value, void>::type Set(T&& p, U&& v)
			{
				GetArg(std::forward<T>(p)) = v;
			}

			template <typename T, typename U>
			static typename std::enable_if<!std::is_convertible<U, decltype(GetArg(std::declval<T>()))>::value, void>::type Set(T&&, U&&)
			{
		...
		{
			template <typename current_arg, typename T> static void AssignArg(current_arg, T)
			{
			}
		};
		...
		template <int arg_index, typename... valuelist> static void Assign(std::tuple<valuelist...>)
		{
		}
	...
	namespace placeholders
	{
		...
		helper::ArgValue<ArgType, std::is_placeholder<PlaceHolder>::value> operator<=(PlaceHolder, ArgType&& arg)
		{
...
namespace fakeit
{
	...
	template <unsigned int index, typename... arglist> class MatchersCollector
	{
		...
		template <typename MatcherCreatorT, typename = void> struct IsMatcherCreatorTypeCompatible : std::false_type
		{
		};
		...
		struct IsMatcherCreatorTypeCompatible<
			MatcherCreatorT,
			typename std::enable_if<MatcherCreatorT::template IsTypeCompatible<NakedArgType<index>>::value, void>::type> : std::true_type
		{
		};
		...
		typename std::enable_if<
			!std::is_same<AnyMatcher, typename naked_type<Head>::type>::value && !IsMatcherCreatorTypeCompatible<typename naked_type<Head>::type>::value &&
		...
		template <typename Head>
		typename std::enable_if<IsMatcherCreatorTypeCompatible<typename naked_type<Head>::type>::value, void>::type CollectMatchers(Head&& creator)
		{
			TypedMatcher<NakedArgType<index>>* d = creator.template createMatcher<NakedArgType<index>>();
			_matchers.push_back(d);
		}

		template <typename Head> typename std::enable_if<std::is_same<AnyMatcher, typename naked_type<Head>::type>::value, void>::type CollectMatchers(Head&&)
		{
			TypedMatcher<NakedArgType<index>>* d = Any().template createMatcher<NakedArgType<index>>();
			_matchers.push_back(d);
		}

		template <typename Head, typename... Tail> void CollectMatchers(Head&& head, Tail&&... tail)
		{
...
namespace fakeit
{
								 ...
								 private Invocation::Matcher
	{
		...
		struct Context : Destructible
		{
			...
			virtual void addMethodInvocationHandler(
				typename ActualInvocation<arglist...>::Matcher* matcher, ActualInvocationHandler<R, arglist...>* invocationHandler) = 0;
		...
		typename std::enable_if<sizeof...(matcherCreators) == sizeof...(arglist), void>::type setMatchingCriteria(matcherCreators&&... matcherCreator)
		{
	...
	template <typename R, typename... arglist> class MockingContext : public MethodMockingContext<R, arglist...>
	{
		...
		public:
		MockingContext(typename MethodMockingContext<R, arglist...>::Context* stubbingContext) : MethodMockingContext<R, arglist...>(stubbingContext)
		{
		}
		...
		template <class... arg_matcher> MockingContext<R, arglist...>& Using(arg_matcher&&... arg_matchers)
		{
	...
	template <typename... arglist> class MockingContext<void, arglist...> : public MethodMockingContext<void, arglist...>
	{
		...
		public:
		MockingContext(typename MethodMockingContext<void, arglist...>::Context* stubbingContext) : MethodMockingContext<void, arglist...>(stubbingContext)
		{
		}
		...
		template <class... arg_matcher> MockingContext<void, arglist...>& Using(arg_matcher&&... arg_matchers)
		{
	...
	{
		public:
		DtorMockingContext(MethodMockingContext<void>::Context* stubbingContext) : MethodMockingContext<void>(stubbingContext)
		{
		}
...
namespace fakeit
{
	...
	template <typename C, typename... baseclasses> class MockImpl : private MockObject<C>, public virtual ActualInvocationsSource
	{
		public:
		...
		MockImpl(FakeitContext& fakeit, C& obj, bool isSpy) : _instanceOwner(isSpy ? nullptr : asFakeObject(&obj)), _proxy{obj}, _fakeit(fakeit)
		{
		}
		...
		template <typename R, typename... arglist> class MethodMockingContextBase : public MethodMockingContext<R, arglist...>::Context
		{
			protected:
		...
		template <typename R, typename... arglist> class MethodMockingContextImpl : public MethodMockingContextBase<R, arglist...>
		{
			protected:
			...
			template <typename... T> [[noreturn]] std::function<R(arglist&...)> getOriginalMethodCopyArgsInternal(long)
			{
				std::abort();
			}
		...
		template <int id, typename R, typename... arglist> class UniqueMethodMockingContextImpl : public MethodMockingContextImpl<R, arglist...>
		{
			protected:
...
namespace fakeit
{
	...
	template <typename C, typename... baseclasses> class Mock : public ActualInvocationsSource
	{
		...
		template <class DATA_TYPE, typename... arglist, class = typename std::enable_if<std::is_member_object_pointer<DATA_TYPE C::*>::value>::type>
...
namespace fakeit
{
	...
	class WhenFunctor
	{
		...
		template <typename R, typename... arglist> MethodProgress<R, arglist...> operator()(const StubbingContext<R, arglist...>& stubbingContext)
		{
...
namespace fakeit
{
	...
	struct InvocationUtils
	{
		...
		static void collectInvocationSources(std::vector<ActualInvocationsSource*>& into, const ActualInvocationsSource& mock, const list&... tail)
		{
		...
		template <typename... list> static void collectSequences(std::vector<Sequence*>& vec, const Sequence& sequence, const list&... tail)
		{
...
namespace fakeit
{
	struct MatchAnalysis
	{
		...
		static void collectMatchedInvocations(std::vector<Invocation*>& actualSequence, std::vector<Invocation*>& matchedInvocations, int start, int length)
		{
...
namespace fakeit
{
	...
	struct SequenceVerificationExpectation
	{
		...
		SequenceVerificationExpectation(VerificationEventHandler& fakeit, InvocationsSourceProxy mocks, std::vector<Sequence*>& expectedPattern)
			: _fakeit(fakeit), _involvedInvocationSources(mocks), _expectedPattern(expectedPattern), _expectedCount(-1), _expectAnything(false), _line(0),
			  _isVerified(false)
		{
		}
		...
		template <typename... list> std::vector<Sequence*>& collectSequences(std::vector<Sequence*>& vec, const Sequence& sequence, const list&... tail)
		{
			vec.push_back(&const_cast<Sequence&>(sequence));
		...
		void handleExactVerificationEvent(VerificationEventHandler& verificationErrorHandler, std::vector<Invocation*> actualSequence, int count)
		{
		...
		void handleAtLeastVerificationEvent(VerificationEventHandler& verificationErrorHandler, std::vector<Invocation*> actualSequence, int count)
		{
...
namespace fakeit
{
	...
	class SequenceVerificationProgress
	{
		...
		SequenceVerificationProgress(FakeitContext& fakeit, InvocationsSourceProxy sources, std::vector<Sequence*>& allSequences)
...
namespace fakeit
{
	...
	class UsingProgress
	{
		...
		template <typename... list> void collectSequences(std::vector<fakeit::Sequence*>& vec, const fakeit::Sequence& sequence, const list&... tail)
		{
		...
		template <typename... list> SequenceVerificationProgress Verify(const fakeit::Sequence& sequence, const list&... tail)
		{
...
namespace fakeit
{
	class VerifyNoOtherInvocationsFunctor
	{
		...
		template <typename... list> VerifyNoOtherInvocationsVerificationProgress operator()(const ActualInvocationsSource& head, const list&... tail)
		{
...
namespace fakeit
{
	...
	class SpyFunctor
	{
		...
		template <typename R, typename... arglist> void spy(const SpyingContext<R, arglist...>&, long)
		{
			static_assert(
				!std::is_same<R, R>::value,
				"Spy() cannot accept move-only args, use SpyWithoutVerify() instead which is able to forward these args but then they won't be available for "
				"Verify().");
		}
...
namespace fakeit
{
	...
	class UnverifiedFunctor
	{
		...
		template <typename... list> UnverifiedInvocationsSource operator()(const ActualInvocationsSource& head, const list&... tail)
		{
			std::vector<ActualInvocationsSource*> allMocks{&InvocationUtils::remove_const(head), &InvocationUtils::remove_const(tail)...};
			InvocationsSourceProxy aggregateInvocationsSource{new AggregateInvocationsSource(allMocks)};
...
#define OVERLOADED_METHOD_PTR(mock, method, prototype) fakeit::Prototype<prototype>::MemberType<typename MOCK_TYPE(mock)>::get(&MOCK_TYPE(mock)::method)
