% ============================================================
%  Beamer презентация – Дипломна защита
%  Compile-time ORM библиотека на C++
%  Алекс Цветанов  |  ТУ–София  |  2026
%  Компилация: lualatex presentation.tex (два пъти за ToC)
% ============================================================
\documentclass[aspectratio=169,12pt]{beamer}

% ── Кирилица и езикова поддръжка (LuaLaTeX – native UTF-8) ─
\usepackage{fontspec}
\usepackage{polyglossia}
\setdefaultlanguage{bulgarian}
\setotherlanguage{english}
% Windows system fonts — гарантирано присъстват, пълна Кирилица
\setmainfont{Times New Roman}
\setsansfont{Arial}
\setmonofont[Scale=0.88]{Courier New}
% Polyglossia изисква явна декларация за всяко семейство
\newfontfamily\cyrillicfont{Times New Roman}
\newfontfamily\cyrillicfontsf{Arial}
\newfontfamily\cyrillicfonttt{Courier New}

% ── Тема и цветове (TU-Sofia blue) ─────────────────────────
\usetheme{Madrid}
\definecolor{tusofia}{RGB}{0,86,145}
\definecolor{tusofiadark}{RGB}{0,60,105}
\definecolor{tusofialight}{RGB}{220,235,248}

\setbeamercolor{palette primary}{bg=tusofia,fg=white}
\setbeamercolor{palette secondary}{bg=tusofiadark,fg=white}
\setbeamercolor{palette tertiary}{bg=tusofia!70,fg=white}
\setbeamercolor{palette quaternary}{bg=tusofia,fg=white}
\setbeamercolor{structure}{fg=tusofia}
\setbeamercolor{block title}{bg=tusofia,fg=white}
\setbeamercolor{block body}{bg=tusofialight,fg=black}
\setbeamercolor{alerted text}{fg=red!70!black}

% ── Шрифт за code ──────────────────────────────────────────
\usepackage{listings}
\usepackage{xcolor}

\definecolor{cppblue}{RGB}{0,0,170}
\definecolor{cppgreen}{RGB}{0,120,0}
\definecolor{cppred}{RGB}{170,0,0}
\definecolor{cppgray}{RGB}{110,110,110}

\lstset{
    language=C++,
    basicstyle=\fontsize{6}{7.5}\ttfamily,
    keywordstyle=\color{cppblue}\bfseries,
    stringstyle=\color{cppred},
    commentstyle=\color{cppgreen}\itshape,
    identifierstyle=\color{black},
    showstringspaces=false,
    breaklines=true,
    breakatwhitespace=true,
    columns=flexible,
    keepspaces=true,
    tabsize=4,
    frame=single,
    framerule=0.4pt,
    rulecolor=\color{cppgray},
    xleftmargin=0.4em,
    xrightmargin=0.2em,
    morekeywords={constexpr,requires,concept,auto,nullptr,noexcept,
                  select,insert,update,deleteq,where,order_by,group_by,
                  limit,prepare,execute,field,Placeholder,ph,
                  IndexedPlaceholder,static_assert,using,namespace},
}

% ── TikZ ───────────────────────────────────────────────────
\usepackage{tikz}
\usetikzlibrary{arrows.meta,positioning,shapes.geometric,fit,calc,decorations.pathreplacing}

\tikzset{
    layerbox/.style={draw=tusofia, rounded corners=3pt, fill=tusofialight,
        align=center, text width=3.4cm, font=\footnotesize,
        minimum width=3.8cm, minimum height=0.55cm, inner sep=3pt},
    appbox/.style={layerbox, fill=green!12, draw=green!60!black},
    ormline/.style={-Latex, thick, color=tusofia},
    capbox/.style={draw=orange!70!black, rounded corners=3pt, fill=orange!10,
        align=center, text width=1.7cm, font=\scriptsize,
        minimum width=1.9cm, minimum height=0.55cm, inner sep=3pt},
}

% ── Графики ────────────────────────────────────────────────
\usepackage{graphicx}
\usepackage{booktabs}
\usepackage{tabularx}

% ── Метаданни ──────────────────────────────────────────────
\title[Compile-time ORM за C++]{%
    Compile-time Object-Relational Mapping\\[0.2em]
    библиотека на C++}
\subtitle{Дипломна работа}
\author[Алекс Цветанов]{%
    Алекс Цветанов, фак.~№~121222225\\[0.3em]
    {\footnotesize Научен ръководител: доц.~д-р~инж.~Галя Павлова}}
\institute[ТУ--София]{%
    Технически университет~--~София\\
    Факултет „Компютърни системи и технологии"\\
    Компютърно и софтуерно инженерство}
\date{2026}

% ══════════════════════════════════════════════════════════
\begin{document}
% ══════════════════════════════════════════════════════════

% ── Заглавна страница ──────────────────────────────────────
\begin{frame}
    \titlepage
\end{frame}

% ── Съдържание ─────────────────────────────────────────────
% ══════════════════════════════════════════════════════════
\section{Мотивация}
% ══════════════════════════════════════════════════════════

\begin{frame}{Проблемът с runtime заявките}
    \begin{columns}[T]
        \begin{column}{0.54\textwidth}
            \textbf{Традиционен подход:}
            \begin{itemize}\small
                \item Заявките се описват като \alert{низове}
                \item Валидиране едва при изпълнение
                \item Грешни имена на полета → runtime crash
                \item Несъвместими типове → late discovery
                \item Смяна на backend → \alert{пълно пренаписване}
            \end{itemize}
            \vspace{0.15em}
            \textbf{Изследователски въпрос:}\\
            \textit{Може ли компилаторът да поеме ролята на
            първи валидатор на заявките?}
        \end{column}
        \begin{column}{0.43\textwidth}
            \small
            \begin{tabularx}{\linewidth}{@{}XX@{}}
                \toprule
                \textbf{Runtime} & \textbf{Compile-time}\\
                \midrule
                Низ заявки &
                    \texttt{constexpr} IR\\
                {Грешка при\newline изпълнение} & {Грешка при\newline \alert{компилация}}\\
                Late discovery &
                    Early detection\\
                \bottomrule
            \end{tabularx}
            \vspace{0.15em}
            \begin{alertblock}{Цел}
                {\small Компилаторът поема ролята
                на първи валидатор
                на заявките.}
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Съществуващи решения и позиция на разработката}
    \small
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            \small
            \begin{tabularx}{\linewidth}{@{}llX@{}}
                \toprule
                \textbf{Решение} & \textbf{Подход} & \textbf{Ограничение}\\
                \midrule
                SOCI      & runtime bind & низ-базирани заявки\\
                ODB       & codegen      & external compiler\\
                sqlpp11   & EDSL         & главно SQL сценарии\\
                Hiberlite & ORM          & само SQLite\\
                \bottomrule
            \end{tabularx}
        \end{column}
        \begin{column}{0.45\textwidth}
            \begin{block}{Цел на разработката}
                \begin{itemize}\scriptsize
                    \setlength{\itemsep}{0.15em}
                    \item \texttt{constexpr} query IR без codegen
                    \item Connector contract и capability ограничения
                    \item 7 backend-а и coroutine async слой
                \end{itemize}
            \end{block}
            \begin{alertblock}{Основна теза}
                {\scriptsize Един C++ модел може да бъде compile-time валидиран,
                multi-backend разширяем и practically useful при async натоварвания.}
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════
\section{Архитектура}
% ══════════════════════════════════════════════════════════

\begin{frame}{Слоеста архитектура на библиотеката}
    \begin{columns}[T]
        \begin{column}{0.42\textwidth}
            \centering
            \begin{tikzpicture}[node distance=0.22cm]
                \node[appbox] (app)
                    {\textbf{Приложен слой}\\ entity + query};
                \node[layerbox, below=of app] (ent)
                    {\textbf{Entity слой}\\
                    \texttt{property}, \texttt{relationship}};
                \node[layerbox, below=of ent] (ir)
                    {\textbf{Query IR слой}\\
                    \texttt{field}, \texttt{Rule}, заявки};
                \node[layerbox, below=of ir] (conn)
                    {\textbf{Connector слой}\\
                    {\scriptsize\texttt{connector\_trait<DB>}}};
                \node[layerbox, fill=gray!10, draw=gray!50,
                    below=of conn] (drv)
                    {\textbf{Driver / runtime}\\
                    SQL, BSON, Cypher, CQL};
                \draw[ormline] (app)  -- (ent);
                \draw[ormline] (ent)  -- (ir);
                \draw[ormline] (ir)   -- (conn);
                \draw[ormline] (conn) -- (drv);
            \end{tikzpicture}
        \end{column}
        \begin{column}{0.55\textwidth}
            \textbf{Ключови архитектурни принципа:}
            \begin{itemize}\small
                \item Никакъв виртуален интерфейс\\
                    {\footnotesize trait-базиран, не наследствен contract}
                \item Query IR е \texttt{constexpr}\\
                    {\footnotesize значима валидация при компилация}
                \item Capability tags ограничават\\
                    неподдържаните операции
                \item Смяна на backend = смяна само на\\
                    типа на конектора
            \end{itemize}
            \vspace{0.05em}
            \begin{alertblock}{Централен инвариант}
                Приложният код не комуникира директно с драйвера —
                работи само с C++ типове.
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════
\section{Реализация}
% ══════════════════════════════════════════════════════════

\begin{frame}[fragile]{Минимален compile-time пример}
    \begin{columns}[T]
        \begin{column}{0.54\textwidth}
\begin{lstlisting}
struct User {
  orm::property<int, "id"> id;
  orm::property<std::string, "name"> name;
};

static constexpr auto q =
  orm::select(
    orm::field<&User::id>,
    orm::field<&User::name>)
  .where(orm::field<&User::id>
    == orm::Placeholder<int>{});
\end{lstlisting}
        \end{column}
        \begin{column}{0.42\textwidth}
            \begin{itemize}\small
                \item \texttt{property<T,"name">} носи тип и compile-time име на колона
                \item Заявката се изгражда като \texttt{constexpr} IR, а не като низ
                \item Грешно поле или неподдържана операция се открива рано
            \end{itemize}
            \vspace{0.2em}
            \begin{alertblock}{Ключова идея}
                {\small Компилаторът поема ролята на първи валидатор на заявката.}
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}[fragile]{Connector trait и Capability механизъм}
    \begin{columns}[T]
        \begin{column}{0.52\textwidth}
            \textbf{Минимален connector contract:}
\begin{lstlisting}
template<>
struct orm::connector_trait<MyDB> {
  template<typename T> struct wire_type;
  using cursor_type = ...;

  using supports_transactions = true_type;

  static auto execute(
    MyDB& db, auto query,
    auto&&... params);
};
\end{lstlisting}
        \end{column}
        \begin{column}{0.45\textwidth}
            \begin{itemize}\small
                \item \texttt{is\_connector<DB>} валидира интерфейса при компилация
                \item Capability таговете ограничават неподдържани операции
                \item Смяната на backend е смяна на типа, а не на query API
            \end{itemize}
            \vspace{0.15em}
            \begin{block}{Демонстрирани backend-и}
                {\small SQLite, PostgreSQL, MySQL, MongoDB, Redis, Neo4j, Cassandra}
            \end{block}
        \end{column}
    \end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════
\section{Backend сценарии}
% ══════════════════════════════════════════════════════════

\begin{frame}{Multi-backend материализация}
    \begin{columns}[T]
        \begin{column}{0.50\textwidth}
            {\small Един Query IR → различни backend-и:}
            \begin{tabularx}{\linewidth}{@{}lX@{}}
                \toprule
                \textbf{Backend}  & \textbf{Материализация}\\
                \midrule
                SQLite      & \texttt{WHERE id = ?}\\
                PostgreSQL  & \texttt{WHERE id = \$1}\\
                MySQL       & \texttt{WHERE id = ?}\\
                \midrule
                MongoDB     & BSON filter\\
                Redis       & \texttt{GET}/\texttt{SET}/\texttt{DEL}\\
                Neo4j       & Cypher \texttt{MATCH…WHERE}\\
                Cassandra   & CQL \texttt{SELECT…FROM}\\
                \bottomrule
            \end{tabularx}
        \end{column}
        \begin{column}{0.47\textwidth}
            \textbf{Абстракция на връзките:}\\[0.1em]
            \begin{tabularx}{\linewidth}{@{}lX@{}}
                \texttt{embed}     & MongoDB вложен документ\\
                \texttt{reference} & SQL foreign key / JOIN\\
                                   & Redis: key prefix\\
                                   & Neo4j: edge\\
            \end{tabularx}
            \vspace{0.1em}
            \begin{alertblock}{Принцип}
                {\small C++ моделът е единственият
                \emph{логически} носител на схемата.
                Backend-ът определя физическата
                материализация.}
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}{Асинхронна архитектура на изпълнението}
    \begin{columns}[T,onlytextwidth]
        \begin{column}{0.48\textwidth}
            \centering
            \resizebox{0.92\linewidth}{!}{%
            \begin{tikzpicture}[node distance=0.40cm and 0.35cm]
                \node[layerbox, font=\scriptsize, minimum width=3.0cm, text width=2.6cm] (api)
                    {\textbf{\texttt{async\_db<DB>}}\\ Task<T>\\ фасада};
                \node[layerbox, font=\scriptsize, minimum width=2.5cm, text width=2.1cm, below=of api] (task)
                    {\textbf{Coroutine}\\ suspend\\ resume};
                \node[layerbox, font=\scriptsize, minimum width=2.2cm, text width=1.9cm, below left=of task] (io)
                    {\textbf{IoContext}\\ timers\\ I/O};
                \node[layerbox, font=\scriptsize, minimum width=2.2cm, text width=1.9cm, below right=of task] (pool)
                    {\textbf{Thread}\\ \textbf{Pool}\\ offload\\ resume};
                \draw[ormline] (api) -- (task);
                \draw[ormline] (task) -- (io);
                \draw[ormline] (task) -- (pool);
                \draw[ormline] (io.north east) .. controls +(0,0.20) and +(-0.18,-0.20) .. (task.south west);
                \draw[ormline] (pool.north west) .. controls +(0,0.20) and +(0.18,-0.20) .. (task.south east);
            \end{tikzpicture}%
            }
        \end{column}
        \begin{column}{0.44\textwidth}
            \begin{itemize}\scriptsize
                \item При waiting фаза worker нишката не блокира, а корутината се suspend-ва
                \item Синхронен connector може да се повдигне чрез offload към pool
                \item Native async path остава възможен чрез awaitable слой
                \item Пулове и транзакции запазват RAII семантиката
            \end{itemize}
            \vspace{0.05em}
            \begin{alertblock}{Ключов извод}
                {\scriptsize Async слоят е втора архитектурна ос на библиотеката, а не ad-hoc wrapper.}
            \end{alertblock}
        \end{column}
    \end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════
\section{Верификация}
% ══════════════════════════════════════════════════════════

\begin{frame}{Верификационна стратегия}
    \begin{columns}[T]
        \begin{column}{0.46\textwidth}
            \begin{block}{Доказателствена база}
                \textbf{271 теста} — 100\% преминаване\\
                {\small Compile-time + unit + integration + live backend проверки}
            \end{block}
            \begin{itemize}\scriptsize
                \item Query IR, capability ограничения, миграции и нишкова безопасност
                \item Async инфраструктура: \texttt{Task<T>}, \texttt{ThreadPool}, \texttt{IoContext}, \texttt{async\_db<DB>}
                \item Benchmark suite в \texttt{bench\_async.cpp} за synchronous vs async throughput
            \end{itemize}
        \end{column}
        \begin{column}{0.50\textwidth}
            \centering
            \begin{tikzpicture}[x=1.0cm,y=0.155cm]
                \draw[->, thick] (0,0) -- (0,25.5) node[above] {\small ускорение async/sync};
                \draw[->, thick] (0,0) -- (5.3,0);

                \foreach \y in {5,10,15,20,25}
                {
                    \draw[gray!35] (0,\y) -- (5.0,\y);
                    \node[left] at (0,\y) {\scriptsize \y};
                }

                \foreach \x/\h/\labeltext/\valuetext in {
                    0.7/1.18/{0ns\\yield}/1.18,
                    1.9/3.38/{10~$\mu$s}/3.38,
                    3.1/15.35/{100~$\mu$s}/15.35,
                    4.3/23.36/{1000~$\mu$s}/23.36}
                {
                    \fill[green!28, draw=black] (\x-0.25,0) rectangle (\x+0.25,\h);
                    \node[font=\scriptsize, align=center, above] at (\x,\h) {\valuetext$\times$};
                    \node[font=\scriptsize, align=center, below=6pt] at (\x,0) {\labeltext};
                }
            \end{tikzpicture}
            \vspace{0.05em}
            {\tiny 4 нишки / 100 задачи. При 0ns ефектът е малък; при доминираща waiting фаза async печели силно.}
        \end{column}
    \end{columns}
\end{frame}

% ══════════════════════════════════════════════════════════
\section{Приноси и заключение}
% ══════════════════════════════════════════════════════════

\begin{frame}{Основни резултати и извод}
    \begin{columns}[T]
        \begin{column}{0.54\textwidth}
            \begin{enumerate}\small
                \item Compile-time ORM модел в стандартен C++23 без external codegen
                \item Един C++ entity модел за 7 backend-а
                \item Формален connector contract и capability-базирана разширяемост
                \item Async слой с измерима полза при I/O-dominated натоварвания
                \item Многостепенна верификация: 271 теста + live backend + benchmark evidence
            \end{enumerate}
        \end{column}
        \begin{column}{0.42\textwidth}
            \begin{block}{Основен извод}
                {\small Между runtime ORM и driver-only достъп съществува жизнеспособна архитектурна позиция: compile-time ORM слой с формални contracts и coroutine-базирано изпълнение.}
            \end{block}
            \textbf{Бъдещо развитие:}
            \begin{itemize}\small
                \item C++26 static reflection
                \item native async конектори
                \item по-пълна migration автоматизация
            \end{itemize}
        \end{column}
    \end{columns}
\end{frame}

\begin{frame}[plain]
    \vfill
    \centering
    {\Large\color{tusofia} Благодаря за вниманието!}\\[0.6em]
    {\large Въпроси?}
    \vfill
\end{frame}

\end{document}
