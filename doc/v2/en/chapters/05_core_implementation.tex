...
A crucial implementation detail is that all builder calls are \emph{constant expressions}. The internal \code{orm\_tuple::tuple\_cat} helper uses index-sequence pack expansion rather than \code{std::tuple\_cat}, making the entire chain constexpr-safe on all supported compilers including MSVC. The practical consequence is that every query object can be declared \code{static constexpr}:
...
The \code{\&} operator combines the \code{\_per\_page} and \code{\_page} user-defined literal helpers in either order, producing a \code{Pagification} object whose structure is known at compile time. Declaring a query \code{static constexpr} makes it a zero-cost global constant: the query IR is constructed once and never reconstructed on repeated execution calls.
