...
#define IMPLEMENT_RULE_OPERATOR(op)                                            \
  ...
  constexpr inline Rule<Rule<T1, op1, T2>, #op, Rule<U1, op2, U2>>             \
  operator op(Rule<T1, op1, T2> rule1, Rule<U1, op2, U2> rule2) {              \
    return Rule<Rule<T1, op1, T2>, #op, Rule<U1, op2, U2>>(rule1, rule2);      \
  }

#define IMPLEMENT_MEM_PTR_OPERATOR(op)                                         \
  template <auto P, typename T1>                                               \
  requires(!std::is_same_v<T1, nullptr_t>) constexpr inline auto operator op(  \
      details::mem_ptr<P> a, T1 x) {                                           \
    if constexpr (details::is_mem_ptr_t<T1>)                                   \
      return Rule<typename details::mem_ptr<P>::ptr_t, #op,                    \
                  typename T1::ptr_t>(a.get(), x.get());                       \
    if constexpr (details::is_not_mem_ptr_t<T1>)                               \
      return Rule<typename details::mem_ptr<P>::ptr_t, #op, T1>(a.get(), x);   \
  }                                                                            \
  template <auto P, typename T1>                                               \
  requires(!details::is_mem_ptr_t<T1>) constexpr inline auto operator op(      \
      T1 x, details::mem_ptr<P> a) {                                           \
...
#define IMPLEMENT_MEM_PTR_OPERATOR_WITH_NULL(op)                               \
  ...
  constexpr inline auto operator op(details::mem_ptr<P> a, nullptr_t) {        \
    return Rule<typename details::mem_ptr<P>::ptr_t, #op, nullptr_t>(a.get(),  \
                                                                     nullptr); \
  }                                                                            \
  template <auto P>                                                            \
  constexpr inline auto operator op(nullptr_t, details::mem_ptr<P> a) {        \
    return Rule<nullptr_t, #op, typename details::mem_ptr<P>::ptr_t>(nullptr,  \
                                                                     a.get()); \
  }

namespace webframe::ORM {
template <details::string_literal op1> constexpr inline auto not_op() {
...
template <typename T> struct not_type;

template <details::is_mem_ptr_t T1, details::string_literal op, auto T2>
class Rule<T1, op, details::mem_ptr<T2>>
    : public Rule<T1, op, typename details::mem_ptr<T2>::ptr_t> {};

template <auto T1, details::string_literal op, details::is_mem_ptr_t T2>
class Rule<details::mem_ptr<T1>, op, T2>
    : public Rule<typename details::mem_ptr<T1>::ptr_t, op, T2> {};

template <details::is_not_mem_ptr_t T1, details::string_literal op,
          details::is_not_mem_ptr_t T2>
...
template <typename T1, typename Table1, details::string_literal op,
          details::is_not_mem_ptr_t T2>
...
template <typename T1, details::string_literal op1, typename T2, typename U1,
          details::string_literal op2, typename U2>
constexpr bool are_same(const Rule<T1, op1, T2> &rule1,
                        const Rule<U1, op2, U2> &rule2) {
  // Different operations
  if constexpr (op1.operator std::string_view() !=
                op2.operator std::string_view()) {
    return false;
  }
  // Different operand types
  else if constexpr (
      requires { {rule1._1.operation}; } ^ requires { {rule2._1.operation}; }) {
    return false;
  } else if constexpr (
      requires { {rule1._2.operation}; } ^ requires { {rule2._2.operation}; }) {
    return false;
  } else {
    bool answer = true;
    // Both rules
    if constexpr (
        requires { {rule1._1.operation}; } &&
        requires { {rule2._1.operation}; }) {
    ...
    // Both rules
    if constexpr (
        requires { {rule1._2.operation}; } &&
        requires { {rule2._2.operation}; }) {
...
template <typename T> struct not_type {
  using type = T;
  constexpr static inline auto convert(T a) { return a; };
};

template <auto P> struct not_type<details::mem_ptr<P>> {
...
struct not_type<Rule<T1, op1, T2>> {
  using type = Rule<typename not_type<T1>::type, not_op<op1>(),
                    typename not_type<T2>::type>;
...
template <typename T1, details::string_literal op1, typename T2>
constexpr inline Rule<typename not_type<T1>::type, not_op<op1>(),
                      typename not_type<T2>::type>
operator!(Rule<T1, op1, T2> a) {
