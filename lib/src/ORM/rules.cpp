...
#define IMPLEMENT_RULE_OPERATOR(op)                                                                                                                            \
	template <typename T1, details::string_literal op1, typename T2, typename U1, details::string_literal op2, typename U2>                                    \
...
#define IMPLEMENT_MEM_PTR_OPERATOR(op)                                                                                                                         \
	template <auto P, typename T1>                                                                                                                             \
		requires(!std::is_same_v<T1, nullptr_t>)                                                                                                               \
	...
	template <auto P, typename T1>                                                                                                                             \
		requires(!details::is_mem_ptr_t<T1>)                                                                                                                   \
...
#define IMPLEMENT_MEM_PTR_OPERATOR_WITH_NULL(op)                                                                                                               \
	template <auto P> constexpr inline auto operator op(details::mem_ptr<P> a, nullptr_t)                                                                      \
	{                                                                                                                                                          \
		return Rule<typename details::mem_ptr<P>::ptr_t, #op, nullptr_t>(a.get(), nullptr);                                                                    \
	}                                                                                                                                                          \
	template <auto P> constexpr inline auto operator op(nullptr_t, details::mem_ptr<P> a)                                                                      \
	{                                                                                                                                                          \
...
namespace webframe::ORM
{
	...
	template <details::is_not_mem_ptr_t T1, details::string_literal op, details::is_not_mem_ptr_t T2> class Rule<T1, op, T2> : public IRule
	{
		public:
	...
	template <typename T1, typename Table1, details::string_literal op, details::is_not_mem_ptr_t T2> class Rule<T1 Table1::*, op, T2> : public IRule
	{
		public:
		static constexpr std::string_view operation = op.operator std::string_view();
		T1 Table1::*_1;
	...
	template <details::is_not_mem_ptr_t T1, details::string_literal op, typename Table2, typename T2> class Rule<T1, op, T2 Table2::*> : public IRule
	{
		public:
		static constexpr std::string_view operation = op.operator std::string_view();
		T1 _1;
	...
	template <typename T1, typename Table1, details::string_literal op, typename Table2, typename T2> class Rule<T1 Table1::*, op, T2 Table2::*> : public IRule
	{
		public:
		static constexpr std::string_view operation = op.operator std::string_view();
		T1 Table1::*_1;
	...
	template <typename U1, details::string_literal op, typename U2> std::ostream& operator<<(std::ostream& out, const Rule<U1, op, U2>& rule)
	{
		out << "(";
		if constexpr (requires {
						  {
							  typename details::i_mem_ptr<U1>::Table()
						  };
					  })
		{
			out << typeid(typename details::i_mem_ptr<U1>::type).name() << " " << typeid(typename details::i_mem_ptr<U1>::Table).name() << "::*";
		}
		if constexpr (!requires {
						  {
							  typename details::i_mem_ptr<U1>::Table()
		...
		if constexpr (requires {
						  {
							  typename details::i_mem_ptr<U2>::Table()
						  };
					  })
		{
			out << typeid(typename details::i_mem_ptr<U2>::type).name() << " " << typeid(typename details::i_mem_ptr<U2>::Table).name() << "::*";
		}
		if constexpr (!requires {
						  {
							  typename details::i_mem_ptr<U2>::Table()
...
#undef IMPLEMENT_MEM_PTR_OPERATOR
	...
	template <typename T1, details::string_literal op1, typename T2> struct not_type<Rule<T1, op1, T2>>
	{
	...
	template <typename T> constexpr inline Rule<std::nullptr_t, "exists", T> exists(T a)
	{
		return Rule<std::nullptr_t, "exists", T>(nullptr, a);
	}

	template <typename T> constexpr inline Rule<std::nullptr_t, "not exists", T> not_exists(T a)
	{
