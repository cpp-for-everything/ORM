...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::join(Ts... rules) const {
    using tuple_type = decltype(
        std::tuple_cat(
            ...
            std::make_tuple((std::declval<webframe::ORM::JoinRule<_mode, Table, Ts>>())...)
        )
    );
    using orm_tuple_type = decltype(
    ...
    return select_query<way, Result, orm_tuple_type, Filters, Limits, Grouping, Ordering>(properties_to_select_query, orm_new_tuple, filters, limits, groupings, ordering);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::where(Ts... rules) const {
    ...
    return select_query<way, Result, Rules, orm_tuple_type, Limits, Grouping, Ordering>(properties_to_select_query, joins, orm_new_tuple, limits, groupings, ordering);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::limit(Ts... rules) const {
    ...
    return select_query<way, Result, Rules, Filters, orm_tuple_type, Grouping, Ordering>(properties_to_select_query, joins, filters, orm_new_tuple, groupings, ordering);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::group_by(auto on) const {
    using tuple_type = decltype(
        std::tuple_cat(
            ...
            std::declval<std::tuple<decltype(webframe::ORM::GroupByRule<webframe::ORM::details::orm_tuple<decltype(Ps)...>, decltype(on)>(webframe::ORM::details::orm_tuple(std::make_tuple(Ps...)), on))>>()
        )
    );
    using orm_tuple_type = decltype(
        webframe::ORM::details::orm_tuple(
    ...
    orm_tuple_type orm_new_tuple (std::tuple_cat(groupings.to_tuple(), std::make_tuple(webframe::ORM::GroupByRule<webframe::ORM::details::orm_tuple<decltype(Ps)...>, decltype(on)>(webframe::ORM::details::orm_tuple(std::make_tuple(Ps...)), on))));
    return select_query<way, Result, Rules, Filters, Limits, orm_tuple_type, Ordering>(properties_to_select_query, joins, filters, limits, orm_new_tuple, ordering);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::group_by() const {
    using tuple_type = decltype(
        std::tuple_cat(
            std::declval<Grouping>().to_tuple(), 
            std::declval<std::tuple<decltype(webframe::ORM::GroupByRule<webframe::ORM::details::orm_tuple<decltype(Ps)...>, nullptr_t>(webframe::ORM::details::orm_tuple(std::make_tuple(Ps...)), nullptr))>>()
        )
    );
    using orm_tuple_type = decltype(
        webframe::ORM::details::orm_tuple(
    ...
    orm_tuple_type orm_new_tuple (std::tuple_cat(groupings.to_tuple(), std::make_tuple(webframe::ORM::GroupByRule<webframe::ORM::details::orm_tuple<decltype(Ps)...>, nullptr_t>(webframe::ORM::details::orm_tuple(std::make_tuple(Ps...)), nullptr))));
    return select_query<way, Result, Rules, Filters, Limits, orm_tuple_type, Ordering>(properties_to_select_query, joins, filters, limits, orm_new_tuple, ordering);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::order_by_impl(std::index_sequence<inds...>) const {
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::order_by_helper1() const {
    using tuple_type = decltype(
        std::tuple_cat(
            ...
            std::declval<std::tuple<decltype(webframe::ORM::OrderBy<arg, webframe::ORM::modes::DEFAULT>())>>()
        )
    );
    using orm_tuple_type = decltype(
        webframe::ORM::details::orm_tuple(
    ...
    return select_query<way, Result, Rules, Filters, Limits, Grouping, orm_tuple_type>(properties_to_select_query, joins, filters, limits, groupings, orm_new_tuple);
}
...
constexpr inline auto webframe::ORM::CRUD::select_query<way, Result, Rules, Filters, Limits, Grouping, Ordering>::order_by_helper2() const {
    using tuple_type = decltype(
        std::tuple_cat(
            std::declval<Ordering>().to_tuple(), 
            std::declval<std::tuple<decltype(webframe::ORM::OrderBy<arg, sort>())>>()
        )
    );
    using orm_tuple_type = decltype(
        webframe::ORM::details::orm_tuple(
