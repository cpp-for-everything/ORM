...
template<typename T>
template<size_t i>
size_t webframe::ORM::Table<T>::_get_index_by_column(const std::string_view column) {
...
template<typename T>
template<size_t i>
T& webframe::ORM::Table<T>::_tuple_to_struct(T& output, const typename webframe::ORM::Table<T>::tuple_t& tuple) {
    if constexpr (std::derived_from<pfr::tuple_element_t<i, T>, details::property>) {
...
template<typename T>
template<typename... Ts>
typename webframe::ORM::Table<T>::tuple_t webframe::ORM::Table<T>::to_tuple(Ts... args) {
    tuple_t output;
    return _to_tuple<0>(output, args...);
}

template<typename T>
constexpr size_t webframe::ORM::Table<T>::get_index_by_column(std::string_view column) {
    return _get_index_by_column<0>(column);
}

template<typename T>
T webframe::ORM::Table<T>::tuple_to_struct(const typename webframe::ORM::Table<T>::tuple_t& tuple) {
    T output;
