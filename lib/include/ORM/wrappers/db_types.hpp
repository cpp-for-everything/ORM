...
namespace webframe::ORM
{
    namespace details
    {
        ...
        public:
            ...
            template<typename Y> requires (!std::is_same_v<Y, PrimitiveWrapper<T>>)
        ...
        public:
            ...
            constexpr PrimitiveWrapper(Ts... args) : T(args...) {}
        };
        ...
        template<typename cpp_equivalent>
        class db_type_placeholder : public details::PrimitiveWrapper<cpp_equivalent>
        {
        public:
            using native_type = cpp_equivalent;
            constexpr db_type_placeholder() : details::PrimitiveWrapper<cpp_equivalent>() {}
            template<typename... Ts> \
            constexpr db_type_placeholder(Ts... args) : details::PrimitiveWrapper<cpp_equivalent>(args...) {}
        };
    } // namespace details

#define COMMA ,
#define create_db_type(db_type_name, cpp_equivalent, db_params, db_enum_type, ...) db_params __VA_ARGS__ \
    class db_type_name : public details::PrimitiveWrapper<cpp_equivalent> \
    { \
    public: \
        using native_type = cpp_equivalent; \
        ...
        template<typename... Ts> \
        constexpr db_type_name(Ts... args) : details::PrimitiveWrapper<cpp_equivalent>(args...) {} \
    };
    ...
    template<size_t size =           255> requires (0 <= size && size <=           255) using TINYTEXT = details::TEXT<size>;
    template<size_t size =        65'535> requires (0 <= size && size <=        65'535) using TEXT = details::TEXT<size>;
    template<size_t size =    16'777'215> requires (0 <= size && size <=    16'777'215) using MEDIUMTEXT = details::TEXT<size>;
    template<size_t size = 4'294'967'295> requires (0 <= size && size <= 4'294'967'295) using LONGTEXT = details::TEXT<size>;

    template<size_t size =           255> requires (0 <= size && size <=           255) using TINYBLOB = details::BLOB<size>;
    template<size_t size =        65'535> requires (0 <= size && size <=        65'535) using BLOB = details::BLOB<size>;
    template<size_t size =    16'777'215> requires (0 <= size && size <=    16'777'215) using MEDIUMBLOB = details::BLOB<size>;
    template<size_t size = 4'294'967'295> requires (0 <= size && size <= 4'294'967'295) using LONGBLOB = details::BLOB<size>;
