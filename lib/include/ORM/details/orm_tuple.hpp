...
namespace webframe::ORM::details
{
	...
	template <typename... Ts> class orm_tuple : public std::tuple<Ts...>
	{
		...
		template <size_t ind> using orm_type = typename std::tuple_element<ind, std::tuple<Ts...>>::type;
	...
	template <typename property>
		requires(std::is_member_pointer_v<property>)
	...
	template <typename value>
		requires(!std::is_member_pointer_v<value>)
	...
	template <typename... Ts, typename... Us> class convertible_tuples<webframe::ORM::details::orm_tuple<Ts...>, webframe::ORM::details::orm_tuple<Us...>>
	{
		public:
		using T = webframe::ORM::details::orm_tuple<Ts...>;
		using U = webframe::ORM::details::orm_tuple<Us...>;
		template <size_t... ind> static constexpr bool eval(std::index_sequence<ind...>)
		{
	...
	{
		public:
		using properties = decltype(std::tuple_cat(typename get_properties<Ts>::properties()...));
	...
	template <is_rule T> class get_properties<T>
	{
		public:
		using properties =
			decltype(std::tuple_cat(typename get_properties<decltype(T()._1)>::properties(), typename get_properties<decltype(T()._2)>::properties()));
		using orm_properties = decltype(orm_tuple(properties()));
	};
	...
	template <is_not_orm_placeholder T, typename... Ts> class get_placeholders<std::tuple<T, Ts...>>
	{
		public:
		using placeholders = typename get_placeholders<std::tuple<Ts...>>::placeholders;
		using orm_placeholders = typename get_placeholders<std::tuple<Ts...>>::orm_placeholders;
	...
	template <is_orm_placeholder T, typename... Ts> class get_placeholders<std::tuple<T, Ts...>>
	{
		public:
		using placeholders = decltype(std::tuple_cat(std::tuple<T>(), std::declval<typename get_placeholders<std::tuple<Ts...>>::placeholders>()));
	...
	template <is_orm_placeholder T, typename... Ts> class get_placeholders_and_properties<std::tuple<T, Ts...>>
	{
		public:
		using placeholders =
			decltype(std::tuple_cat(std::tuple<T>(), std::declval<typename get_placeholders_and_properties<std::tuple<Ts...>>::placeholders>()));
		using orm_placeholders = decltype(orm_tuple(placeholders()));
	};

	template <typename T>
	concept is_property_but_not_placeholder = is_property<T> && !is_orm_placeholder<T>;
	template <is_property_but_not_placeholder T, typename... Ts> class get_placeholders_and_properties<std::tuple<T, Ts...>>
	{
		public:
		using placeholders =
			decltype(std::tuple_cat(std::tuple<T>(), std::declval<typename get_placeholders_and_properties<std::tuple<Ts...>>::placeholders>()));
	...
	template <is_not_property_or_placeholder T, typename... Ts> class get_placeholders_and_properties<std::tuple<T, Ts...>>
	{
		public:
		using placeholders = typename get_placeholders<std::tuple<Ts...>>::placeholders;
		using orm_placeholders = typename get_placeholders<std::tuple<Ts...>>::orm_placeholders;
	...
	template <typename... Ts> class get_placeholders_and_properties<orm_tuple<Ts...>>
	{
		public:
...
{
	template <typename T> static constexpr auto Placeholder = &details::placeholder<T>::_;
}
