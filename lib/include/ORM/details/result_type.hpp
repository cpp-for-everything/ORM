...
constexpr auto result_properties = []<typename T>() consteval {
  return details::is_property<T>;
};

template <auto filter, typename... args>
requires requires { {filter.template operator()<int>()}; }
class result_t {
private:
  template <typename arg1, typename... args1>
  static constexpr auto result_t_utils(arg1 a, args1... A) {
    ...
    if constexpr (sizeof...(args1) > 0) {
      return orm_tuple(
          std::tuple_cat(result_t_utils<arg1>(a).to_tuple(),
                         result_t_utils<args1...>(A...).to_tuple()));
    }
  }
