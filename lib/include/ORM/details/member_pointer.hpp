...
namespace webframe::ORM::details
{
	...
	template <typename _Table, typename T> class i_mem_ptr<T _Table::*> : public mem_ptr_detector
	{
		public:
		using Table = _Table;
		using type = T;
...
namespace webframe::ORM
{
	...
	template <typename T1, details::string_literal op, typename T2> class Statement;

	template <details::is_mem_ptr_t T1, details::string_literal op, details::is_not_mem_ptr_t T2> class Statement<T1, op, T2> : public IStatement
	{
		public:
		static constexpr std::string_view operation = op.operator std::string_view();
		typename T1::ptr_t _1;
		T2 _2;
		constexpr Statement()
		{
		}
	...
	template <details::is_mem_ptr_t T1, details::string_literal op, details::is_mem_ptr_t T2> class Statement<T1, op, T2> : public IStatement
	{
		public:
		static constexpr std::string_view operation = op.operator std::string_view();
		typename T1::ptr_t _1;
		typename T2::ptr_t _2;
		constexpr Statement()
		{
		}
	...
	template <details::is_mem_ptr_t T1, details::string_literal op1, typename T2, details::is_mem_ptr_t U1, details::string_literal op2, typename U2>
	constexpr inline bool are_same(const Statement<T1, op1, T2>& Statement1, const Statement<U1, op2, U2>& Statement2)
	{
		...
		// Different operand types
			...
			requires {
				{
					Statement1._1.operation
				};
			} ^
			requires {
				{
					Statement2._1.operation
				};
			})
		{
			return false;
			...
			requires {
				{
					Statement1._2.operation
				};
			} ^
			requires {
				{
					Statement2._2.operation
				};
			})
		{
			return false;
...
namespace webframe::ORM::details
{
	template <auto> class mem_ptr;

	template <auto property>
		requires(!std::is_same_v<decltype(property), nullptr_t>)
	class mem_ptr<property> : public i_mem_ptr<decltype(property)>
	{
		...
		constexpr inline ptr_t get() const
		{
			return property;
		}
	...
	{
		private:
		template <typename T, string_literal lit> struct pseudo_property
		{
			public:
			using var_t = T;
	...
	template <> class mem_ptr<nullptr> : public mem_ptr<&Filler::Null>
	{
		public:
...
namespace webframe::ORM
{
	template <auto Property> constexpr details::mem_ptr<Property> P = details::mem_ptr<Property>();
}
