...
template <typename T>
concept is_query_string =
    std::is_convertible_v<T, std::string_view> && !is_property<T>;
...
template <details::is_mem_ptr_t T1, details::string_literal op,
...
public:
  ...
  constexpr Statement() {}
  constexpr Statement(typename T1::ptr_t a, typename T2::ptr_t b)
      : _1(a), _2(b) {}
};

template <details::is_mem_ptr_t T1, details::string_literal op1, typename T2,
          details::is_mem_ptr_t U1, details::string_literal op2, typename U2>
constexpr inline bool are_same(const Statement<T1, op1, T2> &Statement1,
                               const Statement<U1, op2, U2> &Statement2) {
  ...
  // Different operand types
      ...
      requires { {Statement1._1.operation}; } ^
      requires { {Statement2._1.operation}; }) {
    return false;
  } else if constexpr (
      requires { {Statement1._2.operation}; } ^
      requires { {Statement2._2.operation}; }) {
    return false;
  } else {
    bool answer = true;
    // Both Statements
...
inline std::ostream &operator<<(std::ostream &out,
                                const Statement<U1, op, U2> &statement) {
  out << "(";
...
requires(!std::is_same_v<decltype(property), nullptr_t>) class mem_ptr<property>
    : public i_mem_ptr<decltype(property)> {
public:
  ...
  constexpr inline auto operator=(auto x) const {
    if constexpr (details::is_mem_ptr_t<decltype(x)>)
      return webframe::ORM::Statement<typename details::mem_ptr<property>, "=",
                                      typename decltype(x)::ptr_t>(this->get(),
                                                                   x.get());
    if constexpr (details::is_not_mem_ptr_t<decltype(x)>)
      return webframe::ORM::Statement<typename details::mem_ptr<property>, "=",
                                      decltype(x)>(this->get(), x);
  }
};
...
private:
  ...
  public:
    using var_t = T;
    static constexpr std::string_view _name() { return lit.value; }
  };
