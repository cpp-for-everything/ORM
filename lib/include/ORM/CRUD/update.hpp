...
namespace webframe::ORM::CRUD {
    template<typename Statements, typename Filters,  typename Ordering, typename Limits>
    class update_query : public details::update_query_t {
        ...
        Ordering ordering;
    public:
        using tables = decltype(
            webframe::ORM::details::orm_tuple(
                typename details::unique_tables_t<decltype(
                    std::tuple_cat(
        ...
        constexpr inline auto update(Ts... new_statements) const {
            using new_statements_t = decltype(
                webframe::ORM::details::orm_tuple(std::tuple_cat(statements.to_tuple(), std::tuple<Ts...>(new_statements...)))
            );
            return update_query<new_statements_t, Filters, Ordering, Limits>(webframe::ORM::details::orm_tuple(std::tuple_cat(statements.to_tuple(), std::tuple<Ts...>(new_statements...))), filters, ordering, limits);
        }

        template<typename... Ts>
        constexpr inline auto where(Ts... new_filters) const {
            using new_wheres = decltype(
                webframe::ORM::details::orm_tuple(std::tuple_cat(filters.to_tuple(), std::tuple<Ts...>(new_filters...)))
            );
            return update_query<Statements, new_wheres, Ordering, Limits>(statements, webframe::ORM::details::orm_tuple(std::tuple_cat(filters.to_tuple(), std::tuple<Ts...>(new_filters...))), ordering, limits);
        }
    ...
    public:
        ...
        constexpr inline auto limit(Ts... new_limits) const {
            using new_limits_t = decltype(
                webframe::ORM::details::orm_tuple(std::tuple_cat(limits.to_tuple(), std::tuple<Ts...>(new_limits...)))
            );
            return update_query<Statements, Filters, Ordering, new_limits_t>(statements, filters, ordering, webframe::ORM::details::orm_tuple(std::tuple_cat(limits.to_tuple(), std::tuple<Ts...>(new_limits...))));
        }
...
namespace webframe::ORM {
    static constexpr auto update(auto... Statements) {
        return CRUD::update_query<details::orm_tuple<decltype(Statements)...>, details::orm_tuple<>, details::orm_tuple<>, details::orm_tuple<>>(details::orm_tuple<decltype(Statements)...>(std::tuple(Statements...)), details::orm_tuple(), details::orm_tuple(), details::orm_tuple());
    } 
}