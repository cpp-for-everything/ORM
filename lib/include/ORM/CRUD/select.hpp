...
template <webframe::ORM::modes::select row_way, typename Result, typename Rules,
...
public:
  ...
  constexpr select_query(Result _properties_to_select_query, Rules _joins,
                         Filters _filters, Limits _limits, Grouping _groupings,
                         Ordering _ordering)
      : properties_to_select_query(_properties_to_select_query), joins(_joins),
        filters(_filters), limits(_limits), groupings(_groupings),
        ordering(_ordering) {}
...
public:
  ...
  template <webframe::ORM::modes::select, typename, typename, typename,
...
static constexpr auto select = CRUD::select_query<
    mode,
    decltype(webframe::ORM::details::result_t<
             webframe::ORM::details::result_all,
             decltype(args)...>::create(args...)), // properties to select
