...
// Unique tables utils
    ...
    struct contains<U, std::tuple<Ts...>> {
    ...
    template<typename orm_tuple_type>
    struct unique_first_tables;

    template<>
    struct unique_first_tables<webframe::ORM::details::orm_tuple<>> {
    ...
    template<typename Statement1, typename... Statements>
    struct second_properties<webframe::ORM::details::orm_tuple<Statement1, Statements...>> {
        static constexpr auto empty_tuple() {
    ...
    template<>
    struct second_properties<webframe::ORM::details::orm_tuple<>> {
...
// unique tables using mem_ptr
    template<auto... Ts>
    struct unique_tables;
    
    template<>
    struct unique_tables<> { static constexpr auto empty_tuple() { return type_container<std::tuple<>>(); } };
    
    template<auto T, auto... Ts>
    struct unique_tables<T, Ts...> {
        static constexpr auto empty_tuple() {
            // skip non-property things
            if constexpr (
                !webframe::ORM::details::is_property<decltype(T)> ||
...
// unique tables using tuple
    ...
    template<>
    struct unique_tables_t<std::tuple<>> { 
...
// DB << query operator
    ...
    auto operator << (DB db, const QueryType query) {
        return [db, query](auto... args) requires (params_match<QueryType, decltype(args)...>) {
