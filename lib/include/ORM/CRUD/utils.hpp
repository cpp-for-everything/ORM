...
template <typename U> struct contains<U, std::tuple<>> {
  static constexpr inline bool check() { return false; }
};

template <typename U, typename Tuple>
constexpr bool contains_v = contains<U, Tuple>::check();
...
template <> struct unique_first_tables<webframe::ORM::details::orm_tuple<>> {
  static constexpr auto empty_tuple() { return type_container<std::tuple<>>(); }
  using types = std::tuple<>;
  using orm_tuple_type =
      decltype(webframe::ORM::details::orm_tuple(std::declval<std::tuple<>>()));
};

template <typename Statement1, typename... Statements>
struct unique_first_tables<
    webframe::ORM::details::orm_tuple<Statement1, Statements...>> {
  static constexpr auto empty_tuple() {
    ...
    if constexpr (!webframe::ORM::details::is_property<T> ||
                  webframe::ORM::details::is_orm_placeholder<T>) {
    ...
    if constexpr (webframe::ORM::details::is_property<T> &&
      ...
      if constexpr (Ts::size == 0)
        return type_container<
            std::tuple<typename webframe::ORM::details::i_mem_ptr<T>::Table>>();

      constexpr auto empty_tuple_of_righter_tables =
          unique_first_tables<Ts>::empty_tuple();

      if constexpr (new_type<
                        typename webframe::ORM::details::i_mem_ptr<T>::Table,
                        typename decltype(empty_tuple_of_righter_tables)::type>)
        return type_container<decltype(std::tuple_cat(
            std::tuple<typename webframe::ORM::details::i_mem_ptr<T>::Table>(),
            typename decltype(empty_tuple_of_righter_tables)::type()))>();

      if constexpr (not_new_type<
                        typename webframe::ORM::details::i_mem_ptr<T>::Table,
                        typename decltype(empty_tuple_of_righter_tables)::type>)
  ...
  using orm_tuple_type = decltype(webframe::ORM::details::orm_tuple(
      std::declval<typename decltype(empty_tuple())::type>()));
};

template <typename orm_tuple_type> struct second_properties;

template <typename Statement1, typename... Statements>
struct second_properties<
    webframe::ORM::details::orm_tuple<Statement1, Statements...>> {
  static constexpr auto empty_tuple() {
    ...
    // if constexpr (!is_property)
    {
      ...
      constexpr auto empty_tuple_of_righter_tables =
          second_properties<Ts>::empty_tuple();
  ...
  using types = typename decltype(empty_tuple())::type;
  using orm_tuple_type =
      decltype(webframe::ORM::details::orm_tuple(std::declval<types>()));
};
...
template <auto T, auto... Ts> struct unique_tables<T, Ts...> {
  static constexpr auto empty_tuple() {
    // skip non-property things
    if constexpr (!webframe::ORM::details::is_property<decltype(T)> ||
                  webframe::ORM::details::is_orm_placeholder<decltype(T)>) {
    ...
    if constexpr (webframe::ORM::details::is_property<decltype(T)> &&
      ...
      if constexpr (sizeof...(Ts) == 0)
        return type_container<
            std::tuple<typename webframe::ORM::details::mem_ptr<T>::Table>>();

      constexpr auto empty_tuple_of_righter_tables =
          unique_tables<Ts...>::empty_tuple();

      if constexpr (new_type<
                        typename webframe::ORM::details::mem_ptr<T>::Table,
                        typename decltype(empty_tuple_of_righter_tables)::type>)
        return type_container<decltype(std::tuple_cat(
            std::tuple<typename webframe::ORM::details::mem_ptr<T>::Table>(),
            typename decltype(empty_tuple_of_righter_tables)::type()))>();
...
struct unique_tables_t<webframe::ORM::details::orm_tuple<Ts...>> {
  static constexpr auto empty_tuple() { return type_container<std::tuple<>>(); }
  using types = typename unique_tables_t<std::tuple<Ts...>>::types;
  using orm_tuple_type =
      typename unique_tables_t<std::tuple<Ts...>>::orm_tuple_type;
};
...
struct unique_tables_t<std::tuple<T, Ts...>> {
  static constexpr auto empty_tuple() {
    // skip non-property things
    if constexpr (!webframe::ORM::details::is_property<T> ||
                  webframe::ORM::details::is_orm_placeholder<T>) {
    ...
    if constexpr (webframe::ORM::details::is_property<T> &&
      ...
      constexpr auto empty_tuple_of_righter_tables =
          unique_tables_t<std::tuple<Ts...>>::empty_tuple();
...
concept params_match = requires {
  {
    webframe::ORM::details::convertible_tuples<
        typename Query::properties,
        typename webframe::ORM::details::orm_tuple<args...>> {}
    } -> webframe::ORM::details::pseudo_concept;
};

// DB << query operator
template <typename DB, is_query QueryType>
auto operator<<(DB db, const QueryType query) {
  return [ db, query ](auto... args) requires(
      params_match<QueryType, decltype(args)...>) {
...
enum select { ALL, DISTINCT, DISTINCTROW };

enum join { INNER, LEFT, RIGHT, FULL };
