...
template <modes::join _mode, typename _Table, is_rule rule_t>
...
public:
  ...
  constexpr operator rule_t() { return this->to_rule(); }

  constexpr rule_t to_rule() const { return static_cast<rule_t>(*this); }
};

template <typename _Properties, typename rule_t> class GroupByRule;

template <typename _Properties, is_rule rule_t>
class GroupByRule<_Properties, rule_t> : public rule_t {
public:
  ...
  constexpr GroupByRule(Properties p, auto a, auto b)
      : rule_t(a, b), properties(p) {}

  static constexpr bool has_rule() { return true; }
