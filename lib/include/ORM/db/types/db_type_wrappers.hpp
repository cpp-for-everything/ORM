...
namespace webframe::ORM
{
	...
	template <size_t n> class BIT : public std::bitset<n>
	{
		...
		const native_type& get_const() const
		{
			return *this;
		}
	};
	...
	// Strings
	template <typename T>
	concept Character = std::same_as<T, char> || std::same_as<T, signed char> || std::same_as<T, unsigned char> || std::same_as<T, wchar_t> ||
...
#undef specify
	...
	template <size_t n, size_t p = percision_not_set>
		requires(p <= 24 || p == percision_not_set)
	using FLOAT = webframe::ORM::Primitive<float, "FLOAT", n, p>;

	template <size_t n, size_t p = percision_not_set>
		requires((p >= 25 && p <= 53) || p == percision_not_set)
	...
	template <size_t n, size_t p> using DECIMAL = typename impl::DECIMAL_wrapper<n, p>::type;
	...
	template <typename T> class Nullable : public INullable, public T
	{
		...
		template <typename U>
			requires(!std::is_same_v<U, nullptr_t>)
