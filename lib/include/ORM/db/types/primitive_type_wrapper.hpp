    ...
    namespace details {
        ...
        #define BinaryOperatorWrapper(op, name) \
            ...
            struct name<T> : public Wrapper<T> { \
                template<typename U> \
                inline decltype(T() op U()) operator op (U b) const { \
            ...
            struct name<T> : public Wrapper<T> { \
                template<typename U> \
                inline T operator op (U) const { \
                    static_assert(operator_not_available<T, U>, "error: invalid operands of types ‘T’ and ‘U’ to binary ‘operator"#op"’"); \
                    return this->get(); \
                } \
            }
        ...
        #define BinaryRefOperatorWrapper(op, name) \
            ...
            struct name<T> : public Wrapper<T> { \
                template<typename U> requires (!std::is_base_of_v<std::ios_base, U>) \
                inline T& operator op (U b) { \
                    return (this->get() op b); \
                } \
            }; \
                                                                \
        ...
        #define UnaryPrefixOperatorWrapper(op, name) \
            ...
            struct name; \
                                                                \
            template<typename T> requires requires (T& a) { \
                { op a }; \
            } \
            struct name<T> : public Wrapper<T> { \
            ...
            struct name<T> : public Wrapper<T> { \
                inline T& operator op () { \
                    static_assert(operator_not_available<T>, "error: invalid operand of types ‘T’ to unary prefix ‘operator"#op"’"); \
                    return (this->get()); \
                } \
            }
        ...
        #define UnarySuffixOperatorWrapper(op, name) \
            ...
            struct name<T> : public Wrapper<T> { \
                inline T& operator op (int) { \
                    static_assert(operator_not_available<T>, "error: invalid operand of types ‘T’ to unary suffix ‘operator"#op"’"); \
                    return (this->get()); \
                } \
            }
        ...
        template<typename T>
        struct istreamOperationsWrapper : public Wrapper<T> {
            template<class CharT, class Traits>
            friend inline std::basic_istream<CharT, Traits>& operator >> (std::basic_istream<CharT, Traits>& in, istreamOperationsWrapper<T>& a) {
                return (in >> a.get());
            }
        };

        template<typename T>
        struct ostreamOperationsWrapper : public Wrapper<T> {
            template<class CharT, class Traits>
            friend inline std::basic_ostream<CharT, Traits>& operator << (std::basic_ostream<CharT, Traits>& out, const ostreamOperationsWrapper<T>& a) {
                return (out << a.get());
            }
        };
    }
