...
namespace webframe::ORM
{
	...
	namespace MockDB
	{
		class MockDB
		{
			...
			template <size_t ind_placeholders, typename T1, auto op, typename T2> struct prepare_rule<ind_placeholders, webframe::ORM::Rule<T1, op, T2>>
			{
			...
			template <typename T> static constexpr auto injection_prevent(T val)
			{
				...
				if constexpr (std::is_member_pointer_v<T>)
				{
					...
					if constexpr (!std::is_same_v<webframe::ORM::details::Filler, typename webframe::ORM::details::i_mem_ptr<T>::Table>)
					{
						return std::string(webframe::ORM::details::i_mem_ptr<T>::Table::table_name) + "." +
			...
			template <size_t ind_query, size_t ind_param> void execute_insert_impl(std::stringstream& ss, const auto& query, const auto& tuple_params) const
			{
			...
			template <std::size_t... Is> void for_(auto func, std::index_sequence<Is...>) const
			{
				(func.template operator()<Is>(), ...);
			}

			template <typename T, std::size_t... Is> void for_with(auto func, std::index_sequence<Is...>) const
			{
			...
			inline void execute_select_impl(std::stringstream& ss, const auto& query, const auto& tuple_params) const
			{
				...
				if constexpr (ind_to_select == select_type::Response::size && ind_join < select_type::Joins::size)
				{
					...
					execute_select_impl<ind_to_select, ind_join + 1, ind_filter, ind_limit, ind_grouping, ind_ordering, next_placeholder>(
						ss, query, tuple_params);
				}
				if constexpr (ind_to_select == select_type::Response::size && ind_join == select_type::Joins::size && ind_filter < select_type::Wheres::size)
				{
					ss << delim << "WHERE ";
					...
					execute_select_impl<ind_to_select, ind_join, ind_filter + 1, ind_limit, ind_grouping, ind_ordering, next_placeholder>(
						ss, query, tuple_params);
				}
				if constexpr (
					ind_to_select == select_type::Response::size && ind_join == select_type::Joins::size && ind_filter == select_type::Wheres::size &&
					ind_limit < select_type::Limitations::size)
				{
					ss << delim << "LIMIT " << query.limit_clauses().template get<ind_limit>().get_elements_per_page() << " x "
					   << query.limit_clauses().template get<ind_limit>().get_number_of_page();
				...
				if constexpr (
					ind_to_select == select_type::Response::size && ind_join == select_type::Joins::size && ind_filter == select_type::Wheres::size &&
					ind_limit == select_type::Limitations::size && ind_grouping < select_type::Groupings::size)
				{
					ss << delim << "GROUP BY (";
					for_(
						[&]<size_t i>() { ss << injection_prevent(query.group_clauses().template get<ind_grouping>().properties.template get<i>()); },
					...
					if constexpr (decltype(query.group_clauses().template get<ind_grouping>())::has_rule())
					{
						...
						constexpr size_t next_placeholder =
							prepare_rule<ind_param, decltype(query.group_clauses().template get<ind_grouping>().to_rule())>::run();
						execute_select_impl<ind_to_select, ind_join, ind_filter, ind_limit, ind_grouping + 1, ind_ordering, next_placeholder>(
							ss, query, tuple_params);
					}
					else
					{
				...
				if constexpr (
					ind_to_select == select_type::Response::size && ind_join == select_type::Joins::size && ind_filter == select_type::Wheres::size &&
					ind_limit == select_type::Limitations::size && ind_grouping == select_type::Groupings::size && ind_ordering < select_type::OrderBy::size)
				{
			...
			std::string execute_select(auto query, auto... params) const
			{
				...
				if constexpr (decltype(query)::mode == webframe::ORM::modes::select::DISTINCT)
				{
					ss << "DISTINCT ";
				}
				if constexpr (decltype(query)::mode == webframe::ORM::modes::select::DISTINCTROW)
				{
					ss << "DISTINCTROW ";
				}
				...
				if constexpr (ind_query == std::remove_cvref_t<decltype(query)>::size)
				{
					...
					if constexpr (ind_filter == std::remove_cvref_t<decltype(query)>::where_size)
					{
						if constexpr (ind_ordering == std::remove_cvref_t<decltype(query)>::order_size)
						{
							...
							// if constexpr (ind_limit == std::remove_cvref_t<decltype(query)>::limit_size && ind_param + ind_query + ind_filter + ind_ordering
							// + ind_limit <= std::remove_cvref_t<decltype(tuple_params)>::size) {
			...
			inline void execute_delete_impl(std::stringstream& ss, const auto& query, const auto& tuple_params) const
			{
				...
				if constexpr (ind_filter == std::remove_cvref_t<decltype(query)>::where_size)
				{
					...
					{
						// re-run functionality not available
						// if constexpr (ind_limit == std::remove_cvref_t<decltype(query)>::limit_size && ind_param + ind_filter + ind_ordering + ind_limit <=
			...
			public:
			template <webframe::ORM::CRUD::details::is_query QueryType> inline auto execute(QueryType query, auto... params) const
			{
