...
namespace webframe::ORM
{
	namespace details
	{
		...
		class InsertQuery : public IInsert
		{
			...
			public:
			constexpr auto get_columns() const
			{
				return cols;
			}
			constexpr auto get_update_statements() const
			{
			...
			constexpr InsertQuery(
				typename InsertedColumns::tuple_equivalent _cols,
				typename InsertedValues::tuple_equivalent _vals,
				typename OnDuplicateKeyUpdateAssignments::tuple_equivalent stmts)
				: cols(_cols), vals(_vals), update_statements(stmts)
			{
			}

			template <typename... Statements> constexpr auto on_duplicate_key_update(Statements... stmts) const
			{
		...
		class InsertIntoSelectQuery : public IInsertSelect
		{
			...
			constexpr auto get_select_query() const
			{
			...
			constexpr InsertIntoSelectQuery(
				typename InsertedColumns::tuple_equivalent _cols, SelectQuery query, typename OnDuplicateKeyUpdateAssignments::tuple_equivalent stmts)
				: cols(_cols), selecting_query(query), update_statements(stmts)
			{
			}

			template <typename... Statements> constexpr auto on_duplicate_key_update(Statements... stmts) const
			{
				return InsertIntoSelectQuery<IntoTable, InsertedColumns, SelectQuery, type_constainer<Statements...>>(
					get_columns(), get_select_query(), {stmts...});
			}
		};

		template <auto... mem_ptrs>
			requires(std::is_member_object_pointer_v<decltype(mem_ptrs)> && ...)
		class GenericInsertQuery
		{
			...
			{
				return InsertQuery<IntoTable, InsertedColumns, type_constainer<decltype(arguments)...>, OnDuplicateKeyUpdateAssignments>(
					get_columns(), {arguments...}, get_update_statements());
			}

			template <typename SelectQuery> constexpr auto into(SelectQuery query) const
			{
				return InsertIntoSelectQuery<IntoTable, InsertedColumns, SelectQuery, OnDuplicateKeyUpdateAssignments>(
					get_columns(), query, get_update_statements());
			}
		};
	} // namespace details
	template <auto... mem_ptrs> constexpr auto insert = details::GenericInsertQuery<mem_ptrs...>();
} // namespace webframe::ORM
