...
template <typename T, details::string_literal lit> class property;

template <details::is_inheritable T, details::string_literal lit>
requires(!std::is_same_v<T, std::string> &&
         !std::is_same_v<T, std::string_view>) class property<T, lit>
    : public T, public details::property {
public:
  ...
  template <typename Y>
  requires requires(Y a) { {static_cast<var_t>(a)}; }
...
public:
  ...
  explicit inline operator T() { return *this; }
...
enum RelationshipTypes { one2one, one2many };

template <RelationshipTypes, auto, details::string_literal> class relationship;

template <RelationshipTypes _type, typename P, class T, P T::*external_ref,
          details::string_literal name>
class relationship<_type, external_ref, name> : public P,
                                                public details::relationship {
public:
  using Source = T;
