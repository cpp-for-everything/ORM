...
template <class Type>
constexpr size_array<sizeof(Type) * 3>
type_to_id(identity<Type>,
           std::enable_if_t<!std::is_enum<Type>::value &&
                            !std::is_empty<Type>::value &&
                            !std::is_union<Type>::value> * = 0) noexcept;

template <std::size_t Index>
constexpr auto
    id_to_type(size_t_<Index>,
               if_extension<Index, native_const_ptr_type> = nullptr) noexcept;
template <std::size_t Index>
constexpr auto
    id_to_type(size_t_<Index>,
               if_extension<Index, native_ptr_type> = nullptr) noexcept;
template <std::size_t Index>
...
constexpr auto
    id_to_type(size_t_<Index>,
               if_extension<Index, native_ref_type> = nullptr) noexcept;
...
constexpr std::size_t
type_to_id(identity<Type>,
           std::enable_if_t<std::is_enum<Type>::value> *) noexcept {
...
constexpr std::size_t
type_to_id(identity<Type>,
           std::enable_if_t<std::is_empty<Type>::value> *) noexcept {
...
constexpr std::size_t
type_to_id(identity<Type>,
           std::enable_if_t<std::is_union<Type>::value> *) noexcept {
...
constexpr auto
id_to_type(size_t_<Index>,
           if_extension<Index, native_volatile_ptr_type>) noexcept {
...
///////////////////// Returns array of typeids and zeros if construtor of a type
///accepts sizeof...(I) parameters
template <class T, std::size_t N, std::size_t... I>
constexpr void *
flat_type_to_array_of_type_ids(std::size_t *types,
...
template <class T>
constexpr sequence_tuple::tuple<>
as_flat_tuple_impl(std::index_sequence<>) noexcept {
...
constexpr size_array<N>
resize_dropping_zeros_and_decrementing(size_t_<N>, const Array &a) noexcept {
  size_array<N> result{};
...
constexpr auto
as_flat_tuple_impl_drop_helpers(std::index_sequence<First, I...>,
                                std::index_sequence<INew...>) noexcept {
...
constexpr auto internal_tuple_with_same_alignment() noexcept {
  ...
  static_assert(std::is_trivial<type>::value &&
                ...
                "====================> Boost.PFR: Type can not be reflected "
                "without Loophole or C++17, because it's not POD");
...
template <class T> auto tie_as_tuple(T &val) noexcept {
  static_assert(!std::is_union<T>::value,
                ...
                "section in the docs for more info.");
  static_assert(
      pfr::detail::is_flat_refelectable<T>(
          detail::make_index_sequence<pfr::detail::fields_count<T>()>{}),
...
template <class T, class F, class IndexSeq, class... Fields> struct next_step {
  ...
  template <class Field> operator Field() const {
    pfr::detail::for_each_field_in_depth(t, std::forward<F>(f), IndexSeq{},
                                         identity<Fields>{}...,
                                         identity<Field>{});
...
void for_each_field_in_depth(T &lvalue, F &&f, std::index_sequence<>,
  ...
  offset_based_getter<std::remove_cv_t<std::remove_reference_t<T>>, tuple_type>
      getter;
  std::forward<F>(f)(pfr::detail::make_flat_tuple_of_references(
...
void for_each_field_dispatcher_1(T &t, F &&f, std::index_sequence<I...>,
  ...
  pfr::detail::for_each_field_in_depth(t, std::forward<F>(f),
