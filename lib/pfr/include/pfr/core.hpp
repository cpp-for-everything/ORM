...
get(T &,
  ...
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling pfr::get on non "
                "const non assignable type is allowed only in C++17");
  return 0;
}
#endif
...
using tuple_element = detail::sequence_tuple::tuple_element<
    I, decltype(::pfr::detail::tie_as_tuple(std::declval<T &>()))>;
...
constexpr auto structure_tie(
    ...
    std::enable_if_t<!std::is_assignable<T, T>::value> * = nullptr) noexcept {
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling pfr::structure_tie "
                "on non const non assignable type is allowed only in C++17");
  return 0;
}
#endif
...
template <class T>
constexpr auto
structure_tie(T &&, std::enable_if_t<std::is_rvalue_reference<T &&>::value> * =
                        nullptr) noexcept {
  static_assert(sizeof(T) && false,
                "====================> Boost.PFR: Calling pfr::structure_tie "
                "on rvalue references is forbidden");
  return 0;
}
...
///     struct my_struct { int i, short s; };
///     int sum = 0;
///     pfr::for_each_field(my_struct{20, 22}, [&sum](const auto& field) { sum
///     += field; }); assert(sum == 42);
/// \endcode
...
constexpr detail::tie_from_structure_tuple<Elements...>
tie_from_structure(Elements &...args) noexcept {
